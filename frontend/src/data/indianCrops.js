export const indianCropCategories = [
    {
...
export const allIndianCrops = indianCropCategories.flatMap(g => g.crops);
// Helper to find crop by name
export function findIndianCrop(cropName) {
    if (!cropName)
        return undefined;
    const lower = cropName.trim().toLowerCase();
    return allIndianCrops.find(c => c.name.toLowerCase() === lower || c.nameHi.toLowerCase() === lower || c.name.toLowerCase().includes(lower));
}
