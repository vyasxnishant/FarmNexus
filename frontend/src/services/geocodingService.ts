/**
 * Comprehensive Geocoding & Geographic Distance Resolver for Indian Agriculture Corridors
 */
...
// Major Agricultural Districts, Mandis, Godown Clusters & Terminals across India
...
/**
 * Geocode any Indian location address string into precise geographic coordinates.
 * Returns null if location is genuinely unrecognized (preventing fake markers).
 */
export function geocodeLocation(locationStr?: string | null): GeocodedLocation | null {
  ...
  // 3. Fallback: Parse explicit latitude, longitude pair if present in string "[22.33, 77.09]"
...
/**
 * Calculates road driving distance between two coordinates using the Haversine formula + road curvature factor.
 */
export function calculateGeoDistanceKm(originCoords: [number, number], destCoords: [number, number]): number {
  ...
  // Real-world road transit curvature factor (~1.25x straight-line distance)
